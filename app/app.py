...
def index():
    ...
    cursor.execute('SELECT * FROM mlb_table')
...
def record_view(player_id):
    cursor = mysql.get_db().cursor()
    cursor.execute('SELECT * FROM mlb_table WHERE id=%s', player_id)
    result = cursor.fetchall()
...
def form_update_post(player_id):
    ...
    sql_update_query = """UPDATE mlb_table t SET t.Name = %s, t.Team = %s, t.Position = %s, t.Height_inches = 
...
def form_insert_post():
    ...
    sql_insert_query = """INSERT INTO mlb_table (Name,Team,Position,Height_inches,Weight_lbs,Age) VALUES (%s, %s,%s, %s,%s, %s,%s) """
...
def form_delete_post(player_id):
    cursor = mysql.get_db().cursor()
    sql_delete_query = """DELETE FROM mlb_table WHERE id = %s """
